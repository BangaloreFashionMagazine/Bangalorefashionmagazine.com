...
import { X, Video, Trash2, Calendar, IndianRupee, Share2, Eye, History, BarChart3, UserPlus, Send } from "lucide-react";
...
// Share Stats & Referral Component
const ShareStatsSection = ({ talent }) => {
  ...
  const fetchProfileViews = async () => {
    try {
      const res = await axios.get(`${API}/analytics/talent/${talent.id}/views`);
      setProfileViews(res.data.total_views || 0);
      setWeeklyViews(res.data.weekly_views || 0);
    } catch (err) { console.error(err); }
  };
  ...
  const submitReferral = async () => {
    if (!referralForm.name.trim() || !referralForm.phone.trim() || !referralForm.category) {
      toast({ title: "Please fill name, phone, and category", variant: "destructive" });
      return;
    }
    
    setSubmittingReferral(true);
    try {
      await axios.post(`${API}/talent/referrals/submit`, {
        referrer_id: talent.id,
        referrer_name: talent.name,
        ...referralForm
      });
      toast({ title: "Referral submitted!", description: "Admin will review and contact your referral." });
      setReferralForm({ name: "", phone: "", email: "", instagram_id: "", category: "Model - Female" });
      fetchReferrals();
    } catch (err) {
      toast({ title: "Failed to submit referral", description: err.response?.data?.detail || "Try again", variant: "destructive" });
    }
    setSubmittingReferral(false);
  };
  
  if (loading) return <div className="text-center py-8 text-[#A0A5B0]">Loading stats...</div>;
  
  const categories = [
    "Model - Female", "Model - Male", "Designers", "Photographers", 
    "Makeup Artists", "Hair Stylists", "Stylists", "DJs", 
    "Choreographers", "Casting Coordinators"
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-[#050A14] rounded-xl p-4 text-center border border-[#D4AF37]/10">
          ...
          <p className="text-2xl font-bold text-[#F5F5F0]">{weeklyViews}</p>
          <p className="text-xs text-[#A0A5B0]">Views This Week</p>
        </div>
        <div className="bg-[#050A14] rounded-xl p-4 text-center border border-[#D4AF37]/10">
          <Eye className="w-6 h-6 text-green-400 mx-auto mb-2" />
          <p className="text-2xl font-bold text-[#F5F5F0]">{profileViews}</p>
          <p className="text-xs text-[#A0A5B0]">Total Profile Views</p>
        </div>
        <div className="bg-[#050A14] rounded-xl p-4 text-center border border-[#D4AF37]/10">
          ...
          <p className="text-xs text-[#A0A5B0]">Instagram Shares</p>
        </div>
      </div>
      
      {/* Referrals Brought Section */}
      {referralStats?.referrals?.length > 0 && (
        <div className="bg-green-500/10 rounded-xl p-4 border border-green-500/20">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xl">🎉</span>
            <h3 className="text-green-400 font-bold">You referred {referralStats.total_referrals} talent(s) to BFM!</h3>
          </div>
            ...
            {referralStats.referrals.slice(0, 5).map((ref, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-[#A0A5B0] text-xs">
                  {ref.status === 'approved' ? 'Approved' : ref.status === 'pending' ? 'Pending Review' : ref.status}
                </span>
              </div>
            ))}
          </div>
      ...
      {/* Refer a Talent Section */}
      <div className="bg-[#050A14] rounded-xl p-4 border border-[#D4AF37]/10">
        <div className="flex items-center gap-2 mb-4">
          <UserPlus className="w-5 h-5 text-[#D4AF37]" />
          <h3 className="text-[#F5F5F0] font-bold">Refer a Talent</h3>
        </div>
        <p className="text-[#A0A5B0] text-sm mb-4">Know someone talented? Refer them to BFM and help them get discovered!</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input
            ...
            className="px-3 py-2 bg-[#0A1628] border border-[#D4AF37]/20 rounded-lg text-[#F5F5F0] text-sm"
          />
          <input
            type="tel"
          ...
          <input
            ...
            value={referralForm.instagram_id}
            onChange={(e) => setReferralForm({ ...referralForm, instagram_id: e.target.value })}
            className="px-3 py-2 bg-[#0A1628] border border-[#D4AF37]/20 rounded-lg text-[#F5F5F0] text-sm"
          />
          <select
            value={referralForm.category}
            onChange={(e) => setReferralForm({ ...referralForm, category: e.target.value })}
            className="px-3 py-2 bg-[#0A1628] border border-[#D4AF37]/20 rounded-lg text-[#F5F5F0] text-sm"
          >
            {categories.map(cat => (
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2 bg-[#D4AF37] text-[#050A14] rounded-lg font-bold text-sm hover:bg-[#F5F5F0] disabled:opacity-50"
          >
            <Send size={16} />
      ...
      <div className="bg-[#0A1628] rounded-xl p-6 border border-[#D4AF37]/20">
        ...
        {(showAddForm || editingProduct) && (
          <div className="bg-[#050A14] rounded-lg p-4 mb-6 border border-[#D4AF37]/20">
            ...
            {/* Category Dropdown - Always show all 4 categories */}
            <div className="mb-4">
              <label className="text-[#A0A5B0] text-sm mb-2 block">Product Category *</label>
              <select 
                value={editingProduct ? editingProduct.store_category : newProduct.store_category} 
                onChange={e => editingProduct ? setEditingProduct({...editingProduct, store_category: e.target.value}) : setNewProduct({...newProduct, store_category: e.target.value})}
                className="w-full md:w-auto px-4 py-2 bg-[#0A1628] border border-[#D4AF37]/20 rounded text-[#F5F5F0]"
              >
                {STORE_CATEGORIES.map(cat => (
                  <option key={cat.id} value={cat.id}>
                    {cat.id === "Everyday Chic" ? "👕" : cat.id === "After Dark" ? "✨" : cat.id === "Heritage Luxe" ? "🪔" : "👜"} {cat.label} - {cat.description}
                  </option>
                ))}
              </select>
            </div>
